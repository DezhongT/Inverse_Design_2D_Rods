...
def forward_solver(Kap0, S, eta, BCs, qx_func, qy_func):
    def odefunc(s, q):
        ...
        dq2_ds = q[2]
        dq3_ds = -qx * sin(q[0]) + qy * cos(q[0]) + eta * q[3] * (q[1] + 1./eta * kap0)
        dq4_ds = -qx * cos(q[0]) - qy * sin(q[0]) - eta * q[2] * (q[1] + 1./eta * kap0)
        dq5_ds = cos(q[0])
        dq6_ds = sin(q[0])
...
def numerical_optimization(data, Kap0, eta, BCs, qx_func, qy_func, degree = 15, max_iter = 1000, lr = 0.1):
    s_u = data[:, 0]
    x_u = data[:, 1]
    y_u = data[:, 2]
    ...
    def model_loss(compute_grad = True):
        ...
        def odeLambda(s, lam):
            ...
            matrix = [[zeroR, eta * onesR, zeroR, zeroR, zeroR, zeroR],
                      ...
                      [-qx * cos(q[0]) - qy * sin(q[0]), eta * q[3], zeroR, eta * (q[1] + 1.0/eta * kap0), zeroR, zeroR],
                      [qx * sin(q[0]) - qy * cos(q[0]), -eta * q[2], -eta * (q[1] + 1.0/eta * kap0), zeroR, zeroR, zeroR],
...
def forward_solver_magnetic(Kap0, S, eta, BCs, B):
    def odefunc(s, q):
        kap0 = Kap0(s)
        Bx = B[0]
        By = B[1]
        dq1_ds = eta * q[1] +  kap0 - (By * cos(q[0]) - Bx * sin(q[0]))
        dq2_ds = q[2]
        dq3_ds =  q[3] * (eta * q[1] +  kap0 - (By * cos(q[0]) - Bx * sin(q[0])))
        dq4_ds = -q[2] * (eta * q[1] +  kap0 - (By * cos(q[0]) - Bx * sin(q[0])))
        dq5_ds = cos(q[0])
        dq6_ds = sin(q[0])
        dqds = np.vstack((dq1_ds, dq2_ds, dq3_ds, dq4_ds, dq5_ds, dq6_ds))
        return dqds
...
def numerical_optimization_magnetic(data, Kap0, eta, BCs, B, degree = 15, max_iter = 1000, lr = 0.1):
    s_u = data[:, 0]
    x_u = data[:, 1]
    y_u = data[:, 2]
    ...
    def model_loss(compute_grad = True):
        ...
        def odeLambda(s, lam):
            ...
            kap0 = np.poly1d(params)
            kap0 = kap0(s)
            matrix = [[By*sin(q[0])+Bx*cos(q[0]), eta * onesR, zeroR, zeroR, zeroR, zeroR],
                      [zeroR, zeroR, onesR, zeroR, zeroR, zeroR],
                      [q[3] * (By*sin(q[0])+Bx*cos(q[0])), eta * q[3], zeroR, eta * q[1]  + kap0 - (By * cos(q[0]) - Bx * sin(q[0])), zeroR, zeroR],
                      [-q[2] * (By*sin(q[0])+Bx*cos(q[0])),  -eta * q[2], - eta * q[1] - kap0 + (By * cos(q[0]) - Bx * sin(q[0])) , zeroR, zeroR, zeroR],
                      [-sin(q[0]), zeroR, zeroR, zeroR, zeroR, zeroR],
                      [cos(q[0]), zeroR, zeroR, zeroR, zeroR, zeroR]]
            matrix = np.array(matrix).reshape(6, 6, -1)
            df_dq_T = np.transpose(matrix, axes = (2, 1, 0)) # (6, 6 , -1)
