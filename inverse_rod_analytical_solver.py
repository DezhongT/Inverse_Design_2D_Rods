import numpy as np
from scipy.integrate import solve_bvp
import matplotlib.pyplot as plt
from numpy import cos, sin
from scipy.interpolate import interp1d
from scipy.io import loadmat

mat_data = loadmat("param_noise.mat")
train_data = mat_data["data"]
params = mat_data["p"].reshape(-1, )
# params = np.ones_like(params)
alpha = 1.0 / train_data[-1, 0]
train_data[:, 0] *= alpha

data = {}
s_tr = train_data[:, 0]
x_tr = train_data[:, 1]
y_tr = train_data[:, 2]

data['s_u'] = s_tr
data['x_u'] = x_tr
data['y_u'] = y_tr

plt.figure(figsize=(7,5))
plt.plot(x_tr,y_tr,'kx',markersize=5, label='Training data')
plt.legend(fontsize=10)
plt.show()


mat_data = loadmat("param.mat")
global eta
eta = float(mat_data["eta"][0, 0])
theta0 = mat_data["theta0"][0, 0]

def model_loss(data, params, compute_grad = True):
    s_u = data['s_u']
    x_u = data['x_u']
    y_u = data['y_u']
    # compute x and y from pde solver
    def odefunc(s, q):
        global eta
        kap0 = np.poly1d(params)
        kap0 = kap0(s)
        dq1_ds = eta * (q[1] + 1.0/eta * kap0)
        dq2_ds = q[2]
        dq3_ds = -cos(q[0]) + eta * q[3] * (q[1] + 1./eta * kap0)
        dq4_ds = sin(q[0]) - eta * q[2] * (q[1] + 1./eta * kap0)
        dq5_ds = cos(q[0])
        dq6_ds = sin(q[0])
        dqds = np.vstack((dq1_ds, dq2_ds, dq3_ds, dq4_ds, dq5_ds, dq6_ds))
        return dqds

    # theta0 = -3.0015
    def boundary_conditions(qa, qb):
        return np.array([qa[0] - theta0, qa[4], qa[5], qb[1], qb[2], qb[3]])

    s = np.linspace(0, 1, 101)
    q_guess = np.zeros((6, s.size))

    sol = solve_bvp(odefunc, boundary_conditions, s, q_guess, max_nodes = 10000, tol = 1e-5)
    s = sol.x
    q = sol.y
    residual = sol.rms_residuals
    if max(residual) > 1e-5:
        print("BVP solver is wrong 1", max(residual))
    q_cubic = interp1d(s, q, kind='cubic')
    s_interp = s_u
    q_interp_cubic = q_cubic(s_interp).T

    x_cubic = interp1d(s_u, x_u, kind='cubic')
    y_cubic = interp1d(s_u, y_u, kind='cubic')
    q_guess = q_cubic(np.linspace(0, 1, 101))


    pred_x = q_interp_cubic[:, 4]
    pred_y = q_interp_cubic[:, 5]

    # pred_x, y = f(kap0, params) -> grad F_params
    ll = 0.5 * ((pred_x - x_u) ** 2).sum(0)
    ll = ll + 0.5 * ((pred_y - y_u) ** 2).sum(0)

    # plt.plot(pred_x, pred_y)
    # plt.plot(x_tr,y_tr,'kx',markersize=5, label='Training data')
    # plt.show()

    if not compute_grad:
        return ll

    ## adjoint method for sensitivity analysis
    # construct the g
    Q = np.eye(6)
    Q[:4, :4] = 0
    # construct ode for lambda
    def odeLambda(s, lam):
        # define the function of df/dq
        q = q_cubic(s)
        zeroR = np.zeros_like(q[0])
        onesR = np.ones_like(q[0])
        # kap0 = params[0] * s**4  + params[1] * s**3 + params[2] * s**2 + params[3] * s+ params[4]
        kap0 = np.poly1d(params)
        kap0 = kap0(s)
        matrix = [[zeroR, eta * onesR, zeroR, zeroR, zeroR, zeroR],
                  [zeroR, zeroR, onesR, zeroR, zeroR, zeroR],
                  [sin(q[0]), eta * q[3], zeroR, eta * (q[1] + 1.0/eta * kap0), zeroR, zeroR],
                  [cos(q[0]), -eta * q[2], -eta * (q[1] + 1.0/eta * kap0), zeroR, zeroR, zeroR],
                  [-sin(q[0]), zeroR, zeroR, zeroR, zeroR, zeroR],
                  [cos(q[0]), zeroR, zeroR, zeroR, zeroR, zeroR]]
        matrix = np.array(matrix).reshape(6, 6, -1)
        df_dq_T = np.transpose(matrix, axes = (2, 1, 0)) # (6, 6 , -1)
        # compute the function of dg/du : g = q.T Q q ; dg_dq = (Q + Q.T) * q
        x_interp = x_cubic(s)
        y_interp = y_cubic(s)
        q[4] -= x_interp
        q[5] -= y_interp
        dg_dq = q.T @ (Q + Q.T)

        f = np.zeros_like(lam)
        for i in range(lam.shape[1]):
            df_dq_t = df_dq_T[i]
            lam_t = lam[:, i]
            f[:, i] = df_dq_t @ lam_t

        f = -f - dg_dq.T

        return f

    def bcLambda(lama, lamb):
        return np.array([lama[1], lama[2], lama[3], lamb[0], lamb[4], lamb[5]])

    s = np.linspace(0, 1, 101)
    lam_guess = np.zeros((6, s.size))
    solL = solve_bvp(odeLambda, bcLambda, s, lam_guess, max_nodes=10000, tol = 1e-5)
    S = solL.x
    lam = solL.y # (6, -1)
    residual = solL.rms_residuals
    if max(residual) > 1e-5:
        # kap0 = fmodel[0](s_u, params=params_unflattened[0])
        # print(kap0)
        print("BVP solver is wrong 2")
        # exit(0)
    # compute the intergral
    df_dp_int = np.zeros((lam.shape[1], params.shape[0]))
    for i in range(df_dp_int.shape[0]):
        s = S[i]
        q = q_cubic(s)
        df_dkap = [1, 0, q[3], -q[2], 0, 0]
        df_dkap = np.array(df_dkap).reshape(-1, 1)
        # kap = params[0] s^4 + params[1] s^3 + params[2] s^2 + params[3] s + params[4]
        # dkap_dp = [s**4, s**3, s**2, s, 1]
        # dkap_dp = np.array(dkap_dp).reshape(1, -1)

        dkap_dp = []
        for j in range(params.shape[0]):
            dkap_dp.append(s**(params.shape[0]-1-j))
        dkap_dp = np.array(dkap_dp).reshape(1, -1)
        lam_t = lam[:, i]
        df_dp = df_dkap @ dkap_dp
        df_dp_int[i, :] = df_dp.T @ lam_t

    # print(params)
    grad = np.trapz(df_dp_int.T, S.T)
    grad = 100 * grad
    if np.linalg.norm(grad) > 1:
        grad = grad/np.linalg.norm(grad)

    # if np.linalg.norm(grad) < 1e-3:
    #     grad = 1e-3 * grad/np.linalg.norm(grad)
    # grad = 0.1 * grad/np.linalg.norm(grad)

    # compute the grad


    # exit(0)

    return ll, grad, q_guess

class AdamOptimizer:
    def __init__(self, learning_rate=0.001, beta1=0.9, beta2=0.999, epsilon=1e-8):
        self.learning_rate = learning_rate
        self.beta1 = beta1
        self.beta2 = beta2
        self.epsilon = epsilon
        self.m = None  # First moment estimate
        self.v = None  # Second moment estimate
        self.t = 0      # Time step

    def update(self, params, gradients):
        if self.m is None:
            self.m = np.zeros_like(params)
            self.v = np.zeros_like(params)

        self.t += 1

        # Update biased first and second moment estimates
        self.m = self.beta1 * self.m + (1 - self.beta1) * gradients
        self.v = self.beta2 * self.v + (1 - self.beta2) * (gradients ** 2)

        # Bias-corrected first and second moment estimates
        m_hat = self.m / (1 - self.beta1 ** self.t)
        v_hat = self.v / (1 - self.beta2 ** self.t)

        # Update parameters
        params -= self.learning_rate * m_hat / (np.sqrt(v_hat) + self.epsilon)


optimizer = AdamOptimizer(learning_rate=0.1)

# def lineSearch(data, ll, grad, params):
#     alpha = 1
#     params_int = params.copy()
#     while alpha > 1e-5:
#         params_new = params_int - alpha * grad
#         ll_new = model_loss(data, params_new, compute_grad=False)
#         if ll_new < ll:
#             break
#         else:
#             alpha = 0.5 * alpha
#
#     if alpha < 1e-5:
#         alpha = 1
#
#     return alpha

iter = 0
while iter < 10000:
    ll, grad, q_guess = model_loss(data, params)
    # alpha  = lineSearch(data, ll, grad, params)
    # params -= alpha * grad
    # clip grad
    # if np.linalg.norm(grad) > 1:
    #     grad = grad/np.linalg.norm(grad)
    # if np.linalg.norm(grad) < 1e-3:
    #     grad = 1e-3 * grad/np.linalg.norm(grad)
    optimizer.update(params, grad)
    # params -= 0.04 * grad
    if ll < 0.001:
        break
    # optimizer.update(params, grad)
    print(params)
    print(grad)
    print(f"Iter: {iter} | loss {ll} | grad {np.linalg.norm(grad)}")
    iter = iter + 1
    # exit(0)

def odefunc(s, q):
    kap0 = np.poly1d(params)
    kap0 = kap0(s)
    # print("q: ", q.shape)
    dq1_ds = eta * (q[1] + 1.0/eta * kap0)
    dq2_ds = q[2]
    dq3_ds = -cos(q[0]) + eta * q[3] * (q[1] + 1./eta * kap0)
    dq4_ds = sin(q[0]) - eta * q[2] * (q[1] + 1./eta * kap0)
    dq5_ds = cos(q[0])
    dq6_ds = sin(q[0])
    dqds = np.vstack((dq1_ds, dq2_ds, dq3_ds, dq4_ds, dq5_ds, dq6_ds))

    return dqds

# theta0 = 1.4241
def boundary_conditions(qa, qb):
    return np.array([qa[0] - theta0, qa[4], qa[5], qb[1], qb[2], qb[3]])

s = np.linspace(0, 1, 101)
q_guess = np.zeros((6, s.size))
sol = solve_bvp(odefunc, boundary_conditions, s, q_guess, tol = 1e-10)
s = sol.x
q = sol.y
residual = sol.rms_residuals
if max(residual) > 1e-5:
    print("BVP solver is wrong")

# Plot the solution
plt.plot(sol.y[4], sol.y[5], '--')
plt.plot(data['x_u'], data['y_u'], 'kx', markersize=5, label='Training data')
plt.xlabel('x')
plt.ylabel('y')
plt.title('Solution of the ODE')
plt.grid(True)
plt.axis('equal')

plt.show()

kap0 = np.poly1d(params)
s = np.linspace(0, 1, 101)
kap0 = kap0(s)


from scipy import io
io.savemat('numerical_temp.mat', {'kap0': kap0, 'params': params})
